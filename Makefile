.DEFAULT_GOAL := all
isort = isort metaproj tests
black = black -S -l 120 --target-version py38 metaproj tests

.PHONY: install-linting
install-linting:
	pip install -r tests/requirements-linting.txt

.PHONY: install-metaproj
install-metaproj:
...
install-testing: install-metaproj
	pip install -r tests/requirements-testing.txt

.PHONY: install-docs
install-docs: install-metaproj
	pip install -U -r docs/requirements.txt

.PHONY: install-benchmarks
install-benchmarks: install-metaproj
