...
from . import exceptions, fscommands, history, resourceobserver, taskhandle
...
class _Project:
    ...
    def get_resource(self, resource_name):
        ...
        if not os.path.exists(path):
            raise exceptions.ResourceNotFoundError(f"Resource <{resource_name}> does not exist")
...
class Project(_Project):
    ...
    def __init__(self, projectroot,
                 fscommands=None, ropefolder=".ropeproject", **prefs):
        """A rope project
...
def _find_module_in_folder(folder, modname):
    ...
    for pkg in packages[:-1]:
        if module.is_folder() and module.has_child(pkg):
            ...
            return None
    if module.is_folder():
        if module.has_child(packages[-1]) and module.get_child(packages[-1]).is_folder():
