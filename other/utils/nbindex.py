...
class NBIndex:
    title = "# Index"
    emph_seen = False
    LETTERS_PER_SECTION: int = 5
    RE_LOCATION = re.compile(r"^#+ (.*)")
    index: dict
    def __init__(self):
        self.index = {}

    def collect_index(self, notebook_name: str):
        index = self.index
    ...
    def main(args):
        ...
        for notebook in args:
            self.collect_index(notebook)
        index_sections = self.index_markdown()
...
def collect_index(notebook_name: str, index_  = None):
    index = index_ or {}
    notebook_path = notebook_name
    fragment = ""
    title = None
