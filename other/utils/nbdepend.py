...
@click.command()
@click.option('--graph', is_flag = True, help="Produce graph")
@click.option('--graph-format', default='svg', help="Graph format (gv, pdf, svg, ...)")
@click.option("--project", help="Project name")
@click.option("--transitive-reduction", is_flag = True, help="Use transitive reduction")
@click.option("--cluster-by-parts", is_flag = True, help="Cluster by parts")
@click.argument("notebooks", nargs='*', help="notebooks to determine dependencies from")
@click.pass_context
def cli(ctx, notebooks, graph, graph_format, project, transitive_reduction, cluster_by_parts):
    click.echo(project)
    if graph:
        draw_notebook_dependencies(
                notebooks,
                graph_format,
        ...
        print_notebook_dependencies(notebooks)
