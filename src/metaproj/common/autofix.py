...
ReplacementT: Union[CSTNode, cst.RemovalSentinel, cst.FlattenSentinel[CSTNode]]

@dataclass
class LintPatch:
    """Represents the diff generated by an auto-fixer for arc lint.
    
    # BaseMetadataProvider[CodeRange] -> WhitespaceInclusivePositionProvider
    # BaseMetadataProvider[CodegenPartial] -> ExperimentalReentrantCodegenProvider
    :param int start_offset: The point in source where the replacement was made.
        Typically this is the position of the replaced node. Available as both a byte offset and a position.
    :param CodePosition start_position:
    :param str original_diff_str:
    :param str patched_diff_str:
    """
    ...
    def get(
        ...
        if isinstance(original_node, cst.Module) and isinstance(replacement_node, cst.RemovalSentinel):
        ...
        while True:
            if possible_statement in codegen_partials:
                ...
                patched_statement: cst.BaseStatement = cst.ensure_type(
                    _replace_or_remove(possible_statement, original_node, replacement_node),
                    cst.BaseStatement,
                )
                original_str: str = partial.get_original_statement_code()
                patched_str: str = partial.get_modified_statement_code(patched_statement)
                return LintPatch(
    ...
    def _minimize_tail(self) -> LintPatch:
        ...
        the size of the patch. This is easier than _minimize_head because we don't
        have to track line/column numbers.

        """
        # this is easier than _minimize_head because we don't have to track line/column
        # numbers.
        matching_tail_len = 0
        for (a, b) in zip(reversed(self.original_diff_str), reversed(self.patched_diff_str)):
            if a != b:
                break
            matching_tail_len += 1
