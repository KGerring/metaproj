...
UNNECESSARY_LAMBDA: str = "The lambda that is wrapping {function} is redundant. " + "It can unwrapped safely and used purely."
...
class NoRedundantLambdaRule(CstLintRule):
    ...
    def _is_simple_parameter_spec(node: cst.Parameters) -> bool:
        if node.star_kwarg is not None or len(node.kwonly_params) > 0 or len(node.posonly_params) > 0 or not isinstance(node.star_arg, cst.MaybeSentinel):
            return False
    ...
    def visit_Lambda(self, node: cst.Lambda) -> None:
        if m.matches(
            ...
            m.Lambda(
                ...
                body=m.Call(args=[m.Arg(value=m.Name(value=param.name.value), star="", keyword=None) for param in node.params.params]),
            ),
        ):
