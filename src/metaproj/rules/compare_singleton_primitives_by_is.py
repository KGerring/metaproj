...
class CompareSingletonPrimitivesByIsRule(CstLintRule):
    ...
    QUALIFIED_SINGLETON_PRIMITIVES: FrozenSet[QualifiedName] = frozenset({QualifiedName(name=f"builtins.{name}", source=QualifiedNameSource.BUILTIN) for name in ("True", "False", "None")})
    ...
    def visit_Comparison(self, node: cst.Comparison) -> None:
        ...
        for target in node.comparisons:
            ...
            if isinstance(operator, (cst.Equal, cst.NotEqual)) and (
                not self.QUALIFIED_SINGLETON_PRIMITIVES.isdisjoint(self.get_metadata(QualifiedNameProvider, left_comp, set()))
                or not self.QUALIFIED_SINGLETON_PRIMITIVES.isdisjoint(self.get_metadata(QualifiedNameProvider, right_comp, set()))
            ):
                needs_report = True
                altered_comparisons.append(target.with_changes(operator=self.alter_operator(operator)))
            else:
    ...
    def alter_operator(self, original_op: Union[cst.Equal, cst.NotEqual]) -> Union[cst.Is, cst.IsNot]:
        return (
            cst.IsNot(
