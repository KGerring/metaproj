...
UNNECESSARY_LITERAL: str = "It's unnecessary to use a list or tuple within a call to {func} since" + " there is literal syntax for this type"
UNNCESSARY_CALL: str = "It's slower to call {func}() than using the empty literal, because" + " the name {func} must be looked up in the global scope in case it has" + " been rebound."
...
class RewriteToLiteralRule(CstLintRule):
    ...
    def visit_Call(self, node: cst.Call) -> None:
        if m.matches(node, m.Call(func=m.Name("tuple") | m.Name("list") | m.Name("set") | m.Name("dict"), args=[m.Arg(value=m.List() | m.Tuple())],),) or m.matches(
            node,
            m.Call(func=m.Name("tuple") | m.Name("list") | m.Name("dict"), args=[]),
        ):

            pairs_matcher = m.ZeroOrMore(m.Element(m.Tuple(elements=[m.DoNotCare(), m.DoNotCare()])) | m.Element(m.List(elements=[m.DoNotCare(), m.DoNotCare()])))
            ...
            if not exp.args:
                ...
                elements = cst.ensure_type(arg, cst.List if isinstance(arg, cst.List) else cst.Tuple).elements
            ...
            if call_name == "tuple":
                ...
                new_node = cst.Dict(
                    elements=[
                        (lambda val: cst.DictElement(val.elements[0].value, val.elements[1].value))(
                            cst.ensure_type(
                                ele.value,
