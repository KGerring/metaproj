...
USE_FSTRING_SIMPLE_EXPRESSION_MAX_LENGTH = 70
...
def _gen_match_simple_expression(module: cst.Module) -> Callable[[cst.CSTNode], bool]:
    def _match_simple_expression(node: cst.CSTNode) -> bool:
        ...
        if (isinstance(node, cst.Tuple) and all(len(module.code_for_node(elm.value)) < USE_FSTRING_SIMPLE_EXPRESSION_MAX_LENGTH for elm in node.elements)) or len(
        ...
        if self.quote == original_node.quote:
            for quo in ["'", '"', "'''", '"""']:
                if quo != original_node.quote and quo not in original_node.raw_value:
                    escaped_string = cst.SimpleString(original_node.prefix + quo + original_node.raw_value + quo)
                    if escaped_string.evaluated_value != original_node.evaluated_value:
                        raise Exception(f"Failed to escape string:\n  original:{original_node.value}\n  escaped:{escaped_string.value}")
                    else:
                        return escaped_string
            raise Exception(f"Cannot find a good quote for escaping the SimpleString: {original_node.value}")
...
class UseFstringRule(CstLintRule):
    ...
    MESSAGE: str = "Do not use printf style formatting or .format(). " + "Use f-string instead to be more readable and efficient. " + "See https://www.python.org/dev/peps/pep-0498/"
    ...
    INVALID = [
        ...
        Invalid(
            ...
            expected_replacement="f\"{obj.this_is_a_very_long_expression(parameter)['a_very_long_key']}\"",
        ),
        Invalid(
    ...
    def visit_BinaryOperation(self, node: cst.BinaryOperation) -> None:
        ...
        extracts = m.extract(
            ...
            m.BinaryOperation(
                ...
                right=m.SaveMatchedNode(
                    m.MatchIfTrue(_gen_match_simple_expression(self.context.wrapper.module)),
                    expr_key,
                ),
            ),
        )
        ...
        if extracts:
            ...
            expressions = [elm.value for elm in expr.elements] if isinstance(expr, cst.Tuple) else [expr]
        ...
        elif m.matches(node, m.BinaryOperation(left=m.SimpleString(), operator=m.Modulo())) and isinstance(cst.ensure_type(node.left, cst.SimpleString).evaluated_value, str):
            self.report(node)
