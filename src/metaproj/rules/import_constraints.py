...
from fixit.common.base import CstContext
...
def _gen_testcase_config(dir_rules: Dict[str, object]) -> LintConfig:
    return LintConfig(
        ...
        rule_config={"ImportConstraintsRule": dir_rules}
    )
...
class _ImportConfig:
    ...
    def from_config(settings_for_dir: Dict[object, object]) -> _ImportConfig:
        ...
        if not isinstance(rule_settings_for_dir, list):
            raise ValueError(f"Invalid entry `{rule_settings_for_dir}`.\n" + "The `rules` setting must be a list of rules.")
    ...
    def match(self, global_name: str) -> _ImportRule:
        for r in self.rules:
            if r.match(global_name):
                return r
        raise AssertionError("No matching rule was found. The wildcard rule should make this impossible.")
...
class ImportConstraintsRule(CstLintRule):
    ...
    Rules can be ``"allow"``, ``"deny"``, ``"allow_global"``, ``"allow_local"``,
    ...
    If ``message`` is passed, it must be a string containing a custom message.
    ...
    MESSAGE: str = "According to the settings for this directory in the .fixit.config.yaml configuration file, " + "'{imported}' cannot be imported from within '{current_file}'."
    ...
    VALID = [
        ...
        Valid(
            "import common",
            config=_gen_testcase_config({"some_dir": {"rules": [["common", "allow"], ["*", "deny"]]}}),
            filename="some_dir/file.py",
        ),
        ...
        Valid(
            "from . import module",
            config=_gen_testcase_config({".": {"rules": [["common.safe", "allow"], ["*", "deny"]]}}),
            filename="common/safe/file.py",
        ),
        Valid(
            "from ..safe import module",
            config=_gen_testcase_config({"common": {"rules": [["common.safe", "allow"], ["*", "deny"]]}}),
        ...
        Valid(
            ...
            config=_gen_testcase_config(
                {
                    "dir_1/dir_2": {"rules": [["common.foo.bar", "allow"], ["*", "deny"]]},
    ...
    INVALID = [
        ...
        Invalid(
            ...
            config=_gen_testcase_config({"some_dir": {"rules": [["common", "deny"], ["*", "allow"]]}}),
            filename="some_dir/file.py",
        ),
        Invalid(
            "from common import bar as not_bar",
            config=_gen_testcase_config({"some_dir": {"rules": [["common.bar", "deny"], ["*", "allow"]]}}),
            filename="some_dir/file.py",
        ),
        # Relative imports
        Invalid(
    ...
    def __init__(self, context: CstContext) -> None:
        ...
        import_constraints_config = self.context.config.rule_config.get(self.__class__.__name__, None)
        
        if import_constraints_config is not None:
            formatted_config: Dict[Path, Dict[object, object]] = self._parse_and_format_config(import_constraints_config)
    ...
    def _parse_and_format_config(
        self,
        import_constraints_config: Dict[str, object]
    ...
    def visit_Import(self, node: cst.Import) -> None:
        self._check_names(node, (get_full_name_for_node_or_raise(alias.name) for alias in node.names))
    ...
    def visit_ImportFrom(self, node: cst.ImportFrom) -> None:
        ...
        if abs_module is not None:
            ...
            if isinstance(names, Sequence):
                self._check_names(node, (f"{abs_module}.{alias.name.value}" for alias in names))
    ...
    def _to_absolute_module(self, module: Optional[str], level: int) -> Optional[str]:
        ...
        if current_dir != self._repo_root and self._repo_root not in current_dir.parents:
            return None
