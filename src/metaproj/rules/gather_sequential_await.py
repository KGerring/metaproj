...
class GatherSequentialAwaitRule(CstLintRule):
    ...
    MESSAGE: str = "Using await in a loop will run async function sequentially. Use " + "asyncio.gather() to run async functions concurrently."
    VALID = [
        Valid(
            """
            async def async_foo():
    ...
    def visit_Await(self, node: cst.Await) -> None:
        ...
        if isinstance(parent, (cst.ListComp, cst.SetComp, cst.GeneratorExp)) and parent.elt is node:
            self.report(node)
