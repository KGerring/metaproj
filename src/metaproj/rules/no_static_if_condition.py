...
class NoStaticIfConditionRule(CstLintRule):
    ...
    MESSAGE: str = "Your if condition appears to evaluate to a static value (e.g. `or True`, `and False`). " + "Please double check this logic and if it is actually temporary debug code."
    VALID = [
        Valid(
            """
    ...
    def _extract_static_bool(cls, node: cst.BaseExpression) -> Optional[bool]:
        ...
        if m.matches(node, m.UnaryOperation(operator=m.Not())):
            sub_value = cls._extract_static_bool(cst.ensure_type(node, cst.UnaryOperation).expression)
